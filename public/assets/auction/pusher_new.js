...
if(typeof toast != 'function'){
  function toast(msg,pos,title){
    title = (typeof title=='undefined') ? '모두플랫폼':title;
    ...
    iziToast.show({
      ...
      image: 'https://modoo24.net/v1/image/sub/know_logo.png',
      imageWidth: 70,
      layout: 2,
      timeout: 3000,
      onClosing: function(){
...
function alarmToast(msg,title,rowid,pos){
  title = (typeof title=='undefined') ? '모두플랫폼':title;
  pos = (typeof pos=='undefined') ? 'bottomRight':pos;
  iziToast.show({
    theme: 'dark',
    icon: 'fas fa-bell',
    imageWidth: 70,
...
if(typeof ajaxErrorST != 'function'){
  function ajaxErrorST(jqXHR ){
    ...
    else if(jqXHR.status != 422 && jqXHR.status != 500 ) {
      iziToast.error({
          message: '잠시후에 이용해주세요',
          position: 'bottomRight'
      });
...
/*
if( parseFloat( jQuery().jquery) < 1.9 ){
...
if ( typeof pusher =='undefined'){
  pusher = new Pusher('13fc7ff1bb9caecd8347', {
    cluster: 'ap3'
  });
}
...
//인터벌함수들 삭제
function clearIntervalOnweb() {
  change_ajax_view = function(){};
  order_get_ajax = function(){}
  alram_cnt = function(){}
  alram_exec = function(){};
}
// start 유저정보삭제
...
$("document").ready( function() {
  ...
  //clear interval
  setTimeout( clearIntervalOnweb, 5000)
  ...
  if( typeof Swal == 'undefined'){
    ...
    css.href='//cdn.jsdelivr.net/npm/@sweetalert2/theme-dark@4/dark.css'
    css.rel ="stylesheet"
  ...
  if (typeof moment =='undefined'){
    ...
    momentscript.src= '/community/assets/stisla/node_modules/moment/moment.js';
    head.appendChild(momentscript)

    momentloscript= document.createElement('script');
    momentloscript.type= 'text/javascript';
    momentloscript.src= '/community/assets/stisla/node_modules/moment/locale/ko.js';
    //head.appendChild(momentloscript)
  }
...
function loadfontawsome(){
  ...
  if ((css(span, 'font-family')) !== 'FontAwesome') {
    ...
    cssfont.href='https://use.fontawesome.com/releases/v5.7.2/css/all.css'
    cssfont.rel ="stylesheet"
    cssfont.integrity="sha384-fnmOCqbTlWIlj8LyTjo7mOUStjsKC4pOpQbqyi7RrhN7udi9RwhKkMHpvLbHG9Sr"
    cssfont.crossOrigin = 'anonymous';
    head.appendChild(cssfont)
  }
...
function loadbootstrap(){
  if( typeof $().modal != 'function'){
    ...
    var cssboot= document.createElement('link');
    cssboot.href='https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.css'
    cssboot.rel ="stylesheet"

    head.appendChild(cssboot)
    head.appendChild(script)
    let alertModal=`
    <div id="alertModal" class="modal" style="height: auto;">
      <a href="#" rel="modal:close" style="top: 1px;right: 1px;"> </a>
...
function setAlarmInit() {
  if( typeof moment == 'undefined'){
    setTimeout(setAlarmInit , 100)
    return;
  }

  let url = '/community/rhksfl/alarm';
  $.ajax({
    url: url,
    method:"GET",
    type:'get',
    ...
    success:function(res)
    {
      alarmObj = res.data.data
      alarmUserId = res.data.userid

      alarmCheckTime();
      channel.bind( 'alarm-event', function(data) {
        let message = data.message;
        if( message.type == 'alarm' && message.command =='add' ) {
          message.data.members.forEach( function (mem, idx) {
            if(mem.aam_mem_idx == alarmUserId ) {
              alarmObj.push(message.data);
              toast(message.data.aa_alram_date,'bottomRight','알람이 등록되었습니다')
            }
          })
        }
      });
      alarmCheckInterval = setInterval( alarmCheckTime, 1000*5 )
    },
    error: function ( err ){
      console.log(err)
      setTimeout(setAlarmInit , 3000)
    },
  });
}
function alarmCheckTime() {
  var now = moment().add(5, 'minutes').format('YYYY-MM-DD HH:mm:ss');
  alarmObj.forEach(function(data, idx){
      if( data.aa_alram_date <= now && data.isAlarmed!=true){
         data['isAlarmed'] = true;
         //moment(data.aa_alram_date, 'YYYY-MM-DD HH:mm:ss').format('HH 시:mm 분')
         alarmToast(data.aa_memo.replace(/\n/g, "<br />"),moment(data.aa_alram_date, 'YYYY-MM-DD HH:mm:ss').format('MM월DD일 HH시mm분'), data.aa_idx ,'topRight')
      }
  });
}
function removeAlarmItem(idx){
  let url = '/community/rhksfl/alarm';
  getpost(url,{aa_idx:idx} , console.log )
}

function alram_popv2(idx){
  $.ajax({
    url: '/community/rhksfl/alarm/form',
    method:"get",
    type:'get',
    data: {order_idx:idx, alarm_type:orderTableType} ,
    dataType:'html',
    success:function(res)
    {
      $("#alert-modal-body").html(res)
      $("#alertModal").modal('show')
      $("input[name='aa_alram_date1']").datepicker();
    },
    error: function ( err ){
      if( typeof callbackError =='undefined') ajaxErrorST(err)
      else callbackError( err )
...
function createAlarm() {
  getpost('/community/rhksfl/alarm/regist', $("#alram_form").serialize(), createAlarmSuccess )
}
function createAlarmSuccess(res){
...
// start. 고객 정보삭제
function setRemovedUserOrderType( ) {
  var location = window.location.pathname
  if ( location == '/admin/order/noface') {
    orderTableType = 'nface'
  //  $("tbody > tr >td:last-child").append("<button type='button' onclick='deleteUser(this)'>정보삭제</button>")
  }
  else if( location == '/admin/order/order') {
    orderTableType = 'order'
  }
  else if( location =='/admin/order/clean_order'){
    orderTableType = 'clean'
    return;
  }
  else  {
    orderTableType ='normal'
    return;
  }
  console.log ( "add event : "+orderTableType +'-event')
	channel.bind( orderTableType +'-event', function(data) {
		let message = data.message;
		if( message.type == 'userinfo' && message.command =='del' ) changeRemoveUserData(message)
	});
  channel.bind( 'order-check-event', function(data) {
    let message = data.message;
    if( message.command =='add' ) {
      console.log ( message.type + "신규등록")
      if ( message.type == orderTableType){
        order_get_ajaxV2( orderTableType)
      }
      var title ='신규'
      if ( message.type == 'order') title += ' 이사오더'
      else if ( message.type == 'nface') title += ' 비대면오더'
      title += '등록'
      iziToast.show({
        title: title,
        displayMode: 0,
...
function order_get_ajaxV2(type){
  if ( type=='nface'){
    console.log ( "get nface data ")
    var lastNo = $("#last_idx").val();

    $.ajax({
      type : "POST"
      , url : "/admin/order/noface"
      , data : { cmd:"order_get_list", idx:lastNo }
      , dataType : "json"
      , success : function(data){
        //console.log(data);

        var pushIdx = data.push_idx;
        var listIdx = $("#list_idx").val();
        var lastIdx = data.last_idx;
        var dataHtml = $(data.html).hide().fadeIn(1500);

        $(".tb_list tbody").prepend( dataHtml );

        // idx값이 없을때 추가
        if(listIdx.indexOf( pushIdx ) == -1){
          var in_list_idx = listIdx + "," + data.push_idx;
          $("#list_idx").val( in_list_idx );
        }

        // idx값이 없을때 추가
        if(lastIdx != null){
          $("#last_idx").val( lastIdx );
        }
      }
    });
  }else if ( type=='order'){
...
function changeRemoveUserData(message){
	if( message.order_type != orderTableType) return false;
	var changeidx = 'IDX-'+ message.data.order_id
  console.log ( changeidx + " removed" )
  if( orderTableType == 'order'){
    $("tr>td:first-child>p").each ( function ( i,v) {
  	    let idx = $(v).text()
  	    if( idx == changeidx ) {
  	        let tr = $(v).closest('tr');
  	        $(':nth-child(5)', tr).css('background-color', '#ffc107')
            $(':nth-child(5)', tr).children('a').text("고객정보삭제됨")
  	        $(':nth-child(9)', tr).css('background-color', '#ffc107').text("고객정보삭제됨")
  	    }
  	})
  }else {
    $("tr>td:first-child>p").each ( function ( i,v) {
        ...
        if( idx == changeidx ) {
            let tr = $(v).closest('tr');
            $(':nth-child(3)', tr).css('background-color', '#ffc107')
            $(':nth-child(3)', tr).children('a').text("고객정보삭제됨")
            $(':nth-child(7)', tr).css('background-color', '#ffc107').text("고객정보삭제됨")
        }
    })
  }
  console.log ( changeidx +' 고객정보삭제' )
  toast(changeidx +' 고객정보삭제','bottomRight','고객정보삭제')
}
...
function deleteUser(btn){
  ...
  if( orderTableType == 'nface'){
    username = $(':nth-child(3)', tr).text()
    tel = $(':nth-child(7)', tr).text()

  }else if( orderTableType == 'order'){
      username = $(':nth-child(5)', tr).text()
      console.log ( username)
      tel = $(':nth-child(9)', tr).text()

    }
  idx = $(btn).data('idx');

  if( typeof idx == 'undefined' || !parseInt(idx) ){
    idx = $(':nth-child(1)', tr).children('p').text().split('-')
    if(typeof idx[1] == 'undefined') {
      alert ('해당번호를 찾을 수 없습니다.')
      return;
    }
    idx = idx[1];
  }
  ...
  Swal.fire({
    title: idx +' : ['+username + ']님의 고객정보['+tel+']를 삭제',
    icon: 'warning',
    showDenyButton: true,
    ...
    if (result.isConfirmed) {
      deleteUserInfoprc(idx, orderTableType)
    }
  })
}
function deleteUserInfoprc( idx, type){
  let url = '/community/rhksfl/order/userinfo/del'
  let data = {id:idx, type:type}
  getpost(url,data , console.log )
}
